...
// A minimal type for the demo, as Chat and ChatMessage are removed
type DemoChatItem = { id: string; title: string };
type DemoMessageItem = { id: string; userId: string; text: string; ts: number };
export function DemoPage() {
  // Minimal state — small demo for AI to extend
  const [users, updateUsers] = useState<User[]>([])
  const [chats, updateChats] = useState<DemoChatItem[]>([])
  const [messages, updateMessages] = useState<DemoMessageItem[]>([])
  ...
  const loadBasics = useCallback(async () => {
    // Mocking chat loading as the endpoint is removed
    const uPage = await api<{ items: User[]; next: string | null }>('/api/users');
    const cPage = { items: [{ id: 'demo-chat-1', title: 'Demo Chat' }], next: null };
    updateUsers(uPage.items)
    updateChats(cPage.items)
  }, [])
  const loadMessages = useCallback(async (chatId: string) => {
    // Mocking message loading
    if (chatId === 'demo-chat-1') {
      updateMessages([
        { id: 'msg1', userId: users.length > 0 ? users[0].id : 'u1', text: 'This is a demo message.', ts: Date.now() }
      ]);
    } else {
      updateMessages([]);
    }
  ...
  const createChat = useCallback(async () => {
    ...
    // Mock chat creation
    const c = { id: `demo-chat-${Date.now()}`, title: title.trim() };
  ...
  const send = useCallback(async () => {
    ...
    const m = { id: `msg-${Date.now()}`, userId: selectedUserId, text: text.trim(), ts: Date.now() };
